...
```
We notice that, for both models where the interaction term was tested individually, neither interaction term was significant. However, both the variables `Gestation` and `smoker` are significant, and should be included in the model despite not having an  interaction. In this case, we would prefer to use the model from f)  and not g).  
