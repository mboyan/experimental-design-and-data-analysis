...
_**Group 3**: Sophie Engels, Boyan Mihaylov and Jonas Schäfer_   
...
par(mfrow=c(1,3))
hist(ff$loglongevity, main="Histogram of Log-Longevity", xlab="Log-Longevity",
     col="lightblue")
qqnorm(ff$loglongevity, main="Normal Q-Q Plot of Log-Longevity")
boxplot(loglongevity ~ activity, data=ff, main="Log-Longevity by sexual acitivity",
        col="lightblue", xlab="Sexual activity", ylab="Log-Longevity")
...
anova(ff_aov); summary(ff_aov)
p_activity <- anova(ff_aov)$'Pr(>F)'[1]
loglongev_by_activity <- tapply(ff$loglongevity, ff$activity, mean)
p_baseline_low <- summary(ff_aov)$coefficients[2,4]
```

The ANOVA strongly suggests that there is a statistically significant difference in log-longevity between the three `activity` levels _isolated_, _low_ and _high_ with p=`r round(p_activity, 4)` $< 0.05$, confirming our first intuition. While deviations between our baseline and the _low_ activity group aren't significant with $p=$ `r round(p_baseline_low, 4)` $> 0.05$, the _high_ activity group has substantial deviations from the baseline mean. The estimated mean `loglongevities` for the three sexual activity groups in order are `r round(loglongev_by_activity[1], 4)`, `r round(loglongev_by_activity[2], 4)` and `r round(loglongev_by_activity[3], 4)`.
...
anova(ff_aocv2); summary(ff_aocv2)
p_activity_2 <- anova(ff_aocv2)$'Pr(>F)'[2]
...
Here, we confirm again that sexual `activity` is a significant factor with p=`r round(p_activity_2, 4)` $< 0.05$. We can notice its effect of decreasing longevity with increasing activity based also on our predictions for the three `activity` groups when fixing `thorax` length of the data set to its mean. The estimated mean `loglongevities` for the three sexual `activity` groups in order are `r round(pred_loglongev[1], 4)` ( _isolated_ ), `r round(pred_loglongev[2], 4)` ( _low_ ) and `r round(pred_loglongev[3], 4)` ( _high_ ).
...
p_interaction <- anova(ancova_model)$'Pr(>F)'[3]
