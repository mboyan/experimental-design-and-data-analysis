...
**c)**
First, we will conduct the poisson regression with program type, math, and their interaction to see if their interaction is significant in the model. 

```{r, fig.height=2.6, fig.width=3}
awardsglm2 = glm(num_awards~prog*math, family=poisson, data=awards)
anova(awardsglm2, test="Chisq")
```
From our findings, we see that the interaction between program type and math score is insignificant, so we do not need to include it in the model. However, both program and math are significant when determining the number of awards, so we will keep those variables in the model and leave out their interaction. 
