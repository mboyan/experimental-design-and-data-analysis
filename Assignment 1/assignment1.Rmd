...
```{r, fig.height=4, fig.width=14, results='hide', warning=FALSE}
library(lme4)

# Check if starter has an effect on acidity
mixedaov_1_pos <- lmer(acidity ~ batch + starter + (1|position), 
                       data=cream_sum)
mixedaov_2_pos <- lmer(acidity ~ batch + (1|starter) + (1|position), 
                       data=cream_sum)
