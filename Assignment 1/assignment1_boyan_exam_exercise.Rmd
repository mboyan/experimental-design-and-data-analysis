...
# 2. Sour cream
```{r}
cream <- read.csv("data/cream.txt", sep="", header=TRUE)
...
contrasts(cream$batch) <- contr.sum
contrasts(cream$position) <- contr.sum
model <- lm(acidity~starter+batch+position, data=cream)
anova(model)
summary(model)
```
## b)
```{r}
xtabs(acidity~starter+batch, data=cream)
interaction.plot(cream$batch, cream$starter, cream$acidity)
```
```{r}
model2 <- lm(acidity~starter+batch, data=cream)
summary(model2)
...
```{r}
library(lme4)
creamlmer <- lmer(acidity~starter+(1|batch), data=cream, REML=FALSE)
summary(creamlmer)
creamlmenr1 <- lmer(acidity~(1|batch), data=cream, REML=FALSE)
anova(creamlmenr1, creamlmer)
```
